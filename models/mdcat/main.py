...
def main():
    ...
    with (h5py.File(dataset_file, 'r') as hdf5_file):
        ...
        model = MultimodalDoubleCoAttentionTransformer(omic_sizes=omics_sizes)
...
if __name__ == '__main__':
    print(f'[{datetime.datetime.now().strftime("%d/%m/%Y - %H:%M")}] MDCAT main started')
    main()
    print(f'[{datetime.datetime.now().strftime("%d/%m/%Y - %H:%M")}] MDCAT main finished')
