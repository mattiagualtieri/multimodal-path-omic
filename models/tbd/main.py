...
from model import MultimodalCoAttentionTransformer
...
def train(epoch, config, device, train_loader, model, loss_function, optimizer):
    ...
    for batch_index, (survival_months, survival_class, censorship, omics_data, patches_embeddings) in enumerate(
        ...
        omics_data = [omics_data['rnaseq'].to(device)]
...
def main():
    ...
    dataset = MultimodalDatasetV2(file_csv, config, use_signatures=False, top_rnaseq=2000)
    ...
    # Model
    omics_sizes = [dataset.rnaseq_size]
...
if __name__ == '__main__':
    print(f'[{datetime.datetime.now().strftime("%d/%m/%Y - %H:%M")}] TBD main started')
    main()
    print(f'[{datetime.datetime.now().strftime("%d/%m/%Y - %H:%M")}] TBD main finished')
